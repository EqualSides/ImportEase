...
export async function buildExportZip(entries: ZipEntryData[]): Promise<Uint8Array> {
  ...
  for (const entry of entries) {
    ...
    } else if (entry.kind === "inspectionGroup") {
      zip.file(
        entry.path,
    ...
    } else if (entry.kind === "departmentType") {
      zip.file(
        entry.path,
        buildExportedDepartmentTypeXml({ listAttrs: entry.listAttrs, records: entry.records })
      );
    } else if (entry.kind === "conditions") {
      zip.file(
        entry.path,
